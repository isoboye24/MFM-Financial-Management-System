...
namespace MFMFMS.Application.Features.Members.Commands.DeleteMember
{
    public class DeleteMemberCommandHandler : IRequestHandler<DeleteMemberCommand>
    {
        ...
        public DeleteMemberCommandHandler(IMemberRepository repository, IUnitOfWork unitOfWork)
        {
            _repository = repository;
            _unitOfWork = unitOfWork;
        }

        public async Task Handle(DeleteMemberCommand request)
        {
            ...
            try
            {
                await _repository.Delete(member);

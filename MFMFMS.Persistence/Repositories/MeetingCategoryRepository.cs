...
namespace MFMFMS.Persistence.Repositories
{
    public class MeetingCategoryRepository : Repository<MeetingCategory>, IMeetingCategoryRepository
    {
        ...
        public async Task<bool> Exists(string name, bool isDeleted = false)
        {
            var exists = await _db.MeetingCategories.Where(x => x.Name == name && x.IsDeleted == isDeleted).AnyAsync();
